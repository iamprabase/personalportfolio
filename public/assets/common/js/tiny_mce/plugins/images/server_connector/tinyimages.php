<?php
session_start();
if ((isset($_SESSION['profilid']))&&(isset($_SESSION['pgstatut']))) {//Si session existe
$profilid = $_SESSION['profilid'];
$pguser = $_SESSION['profilname'];
$pgstatut = $_SESSION['pgstatut'];
$varControle = $_SESSION['controleAdmin'];
$page_lang = $_SESSION['admin_lang'];
$page_paramAjouter = $_SESSION['admin_ajouter']; 
if($varControle != '?donneeOK'){
header("Location: pg,logout.jsp");
}
...
//Fin de controle sйcuritй
class tinyimages {
	
	var $folder = '/files/images';
	//var $folder = '/files/images';
	var $thumbs = '.mini';
	var $originales = '.orig';
	...
	private function mainField() {
		...
		if(count($ret) > 0) {
			...
			foreach ($ret as $val) {
				...
				if ($val['widthOrig'] != 0)
					$addOrig = ' <a href="#" title="Inserer l\'image originale" onclick="addImage(this,\''.$val['orig'].'\','.$val['widthOrig'].'); return false;" alt=" ">Inserer l\'image</a>';
				else
					$addOrig = '';
				$return .= '<div class="item" ondblclick="addImage(this,\''.$val['path'].'\','.$width_for_insert.');" onclick="activateItem(this,\''.$val['path'].'\');"><img src="'.$val['src'].'" width="100" height="100" title="'.$val['name'].'"  alt=" " /><div class="labels">'.$addOrig.'</div><div class="labels">'.$val['name'].'</div><div class="labels">'.$val['width'].'x'.$val['height'].'</div><div class="labels">'.$val['size'].'</div></div>';
			}
		...
		if (isset($_FILES['photoupload'])) {
			...
			if (!$error && !in_array($size[2], array(1, 2, 3, 7, 8) ) ) {
				if($_GET['lng']=='ru') {
					...
					$error = 'Le type de fichier est incorrect. Nous vous recommandons : jpg';
				}
			} else
			...
			if ($error) {
				...
				if(!copy($_FILES['photoupload']['tmp_name'], $source)) {
					...
					if($_GET['lng']=='ru') {
						...
						$result['error'] = 'Fichier non tйlйchargй!';
					}
				} else {
